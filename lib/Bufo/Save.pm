...
sub parse {
    ...
    if ($current) {
        ...
        die "Unsupported legacy save version\n"
          ...
          && !ref( $envelope->{version} )
          && $envelope->{version} eq '1.0.0';
    }
...
sub _normalize {
    ...
    my $definitions;
    if ( exists $u->{definitions} ) {
        die "Invalid custom upgrade definitions\n" unless ref( $u->{definitions} ) eq 'ARRAY';
        my %merged = map { $_->{id} => $_ } @{ $catalog->upgrades };
        my %seen;
        for my $definition ( @{ $u->{definitions} } ) {
            _object( $definition, 'upgrade definition' );
            my $id = _text( $definition->{id}, 'upgrade definition id' );
            die "Duplicate upgrade definition $id\n" if $seen{$id}++;
            $merged{$id} = $definition;
        }
        $catalog = Bufo::Catalog->new(
            generators   => $catalog->generators,
            upgrades     => [ values %merged ],
            achievements => $catalog->achievements
        );
        $definitions = $JSON->decode( $JSON->encode( $u->{definitions} ) );
    }
    ...
    my %events =
      ( golden_bufo_caught => 1, console_opened => 1, map { 'boss_' . $_->{id} => 1 } @bosses );
    ...
    for my $id ( sort keys %{ $catalog->generators } ) {
        ...
        die "Generator $id count exceeds finite cost range\n"
          if log( $definition->{baseCost} ) + $count * log( $definition->{costMultiplier} ) >
          log(1e300);
        my $unlocked =
            exists( $saved->{unlocked} )          ? _boolean( $saved->{unlocked}, "$id.unlocked" )
          : $count > 0 || $definition->{unlocked} ? 1
          :                                         0;
        die "Owned generator $id is locked\n" if $strict && $count && !$unlocked;
        $unlocked = 1 if !$strict && $count;
        my $enabled =
          exists( $saved->{enabled} ) ? _boolean( $saved->{enabled}, "$id.enabled" ) : 1;
        my @boosts;

        if ( exists $saved->{boosts} ) {
            die "Invalid $id boosts list\n" unless ref( $saved->{boosts} ) eq 'ARRAY';
            my %seen;
            for my $boost ( @{ $saved->{boosts} } ) {
                _object( $boost, "$id boost" );
                my $boost_id = _text( $boost->{id}, 'boost id' );
                die "Duplicate boost $boost_id\n" if $seen{$boost_id}++;
                my $multiplier = _number( $boost->{multiplier}, 'boost multiplier' );
                die "Boost multiplier must be positive\n" unless $multiplier > 0;
                push @boosts,
                  {
                    id         => $boost_id,
                    multiplier => $multiplier,
                    active     => _boolean( $boost->{active}, 'boost active' ),
                    source     => _text( $boost->{source}, 'boost source' )
                  };
            }
        }
        $generators{$id} =
          { count => $count, unlocked => $unlocked, enabled => $enabled, boosts => \@boosts };
    }
    my $purchased = _ids( $u->{purchased}, \%upgrade_ids, 'upgrades.purchased' );
    my $unlocked  = _ids( exists( $a->{unlocked} ) ? $a->{unlocked} : [],
        \%achievement_ids, 'achievements.unlocked' );
    my %progress;
    ...
    my $defeated =
      _ids( exists( $b->{defeated} ) ? $b->{defeated} : [], \%boss_ids, 'bosses.defeated' );
    ...
    my $autosave =
      exists( $settings->{autoSave} ) ? _boolean( $settings->{autoSave}, 'autoSave' ) : 1;
    ...
    my $normalized = {
        resources =>
          { bufos => $bufos, totalBufos => $total, baseClickPower => $base, clickCount => $clicks },
        generators => \%generators,
        explorer   => Bufo::ExplorerModel->normalize( $s->{explorer}, now => $now ),
        upgrades   => {
            purchased => $purchased,
            ( defined($definitions) ? ( definitions => $definitions ) : () )
        },
        achievements => {
            unlocked     => $unlocked,
            progress     => \%progress,
        ...
        prestige =>
          { points => $points, lifetimePoints => $lifetime, transcendences => $transcendences },
    ...
    # Validate combined effects before an imported candidate can be persisted.
    Bufo::Game->new( catalog => $self->{catalog}, state => $normalized, now => $now );
    return $normalized;
}
...
sub _text {
    ...
    die "Invalid $name text\n"
      unless defined($value) && !ref($value) && length($value) && length($value) <= 1000;
    return "$value";
}
